...
from model.modeling.DSRL.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
from model.modeling.DSRL.aspp import build_aspp
from model.modeling.DSRL.decoder import build_decoder
from model.modeling.DSRL.backbone import build_backbone
from model.modeling.DSRL.sr_decoder import build_sr_decoder
...
# Created by Y. Kondo at TTI.
class DeepLabx4(nn.Module):
    def __init__(self, backbone='resnet', output_stride=16, num_classes=21,
                 sync_bn=True, freeze_bn=False):
        super(DeepLabx4, self).__init__()
        if backbone == 'drn':
            output_stride = 8
        ...
        self.up_sr_0 = nn.ConvTranspose2d(64, 64, 2, stride=2) 
        self.up_edsr_0 = EDSRConv(64,64)
        self.up_sr_1 = nn.ConvTranspose2d(64, 64, 2, stride=2) 
        self.up_edsr_1 = EDSRConv(64,64)
        self.up_sr_2 = nn.ConvTranspose2d(64, 32, 2, stride=2) 
        self.up_edsr_2 = EDSRConv(32,32)
    ...
    def forward(self, input):
        ...
        x_sr= self.sr_decoder(x, low_level_feat)
        x_seg_up = F.interpolate(x_seg, size=input.size()[2:], mode='bilinear', align_corners=True)
        x_seg_up = F.interpolate(x_seg_up,size=[self.scale_factor*i for i in input.size()[2:]], mode='bilinear', align_corners=True)

        x_sr_up = self.up_sr_0(x_sr)
        x_sr_up=self.up_edsr_0(x_sr_up)

        x_sr_up = self.up_sr_1(x_sr_up)
        x_sr_up=self.up_edsr_1(x_sr_up)
