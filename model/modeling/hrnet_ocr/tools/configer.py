...
class Configer(object):
    ...
    def __init__(self, args_parser=None, configs=None, config_dict=None):
        if config_dict is not None:
            ...
            if not os.path.exists(configs):
                print('Json Path:{} not exists!'.format(configs))
                exit(0)
        ...
        def _set_value(key, value):
            ...
            while len(remained_parts) > 1:
                ...
                if cur_key not in parent_dict:
                    ...
                    print('{} not exists, set as `dict()`.'.format('.'.join(consumed_parts)))
    ...
    def get(self, *key):
        if len(key) == 0:
            ...
            if key[0] in self.params_root:
                ...
                print('{} KeyError: {}.'.format(self._get_caller(), key))
                exit(1)
    ...
    def add(self, key_tuple, value):
        if self.exists(*key_tuple):
            print('{} Key: {} existed!!!'.format(self._get_caller(), key_tuple))
            exit(1)
        ...
        if len(key_tuple) == 1:
            ...
            print('{} KeyError: {}.'.format(self._get_caller(), key_tuple))
            exit(1)

    def update(self, key_tuple, value):
        if not self.exists(*key_tuple):
            print('{} Key: {} not existed!!!'.format(self._get_caller(), key_tuple))
            exit(1)
    ...
    def plus_one(self, *key):
        if not self.exists(*key):
            print('{} Key: {} not existed!!!'.format(self._get_caller(), key))
            exit(1)
