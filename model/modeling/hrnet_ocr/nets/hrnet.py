...
from model.modeling.hrnet_ocr.backbones.backbone_selector import BackboneSelector
from model.modeling.hrnet_ocr.tools.module_helper import ModuleHelper
...
class HRNet_W48_OCR(nn.Module):
    def __init__(self, configer):
        ...
        # pretrained_model_path = "./weights/hrnetv2_w48_imagenet_pretrained.pth"
        # # m_key, u_key = self.backbone.load_state_dict(fix_model_state_dict(torch.load(pretrained_model_path), addition_word='backbone.'), strict=False)
        ...
        from model.modeling.hrnet_ocr.modules.spatial_ocr_block import SpatialGather_Module
        self.ocr_gather_head = SpatialGather_Module(self.num_classes)
        from model.modeling.hrnet_ocr.modules.spatial_ocr_block  import SpatialOCR_Module
    ...
    def forward(self, x_):
        ...
        out_aux = self.aux_sgd(F.interpolate(out_aux, size=(x_.size(2), x_.size(3)), mode="bilinear", align_corners=True))
        out = self.cls_sgd(F.interpolate(out, size=(x_.size(2), x_.size(3)), mode="bilinear", align_corners=True))
        return out, out_aux
